...
public class examen_1_4 {
	public static void main (String []args) {
		...
		{
			//Si empieza o termina con "." no es válido
			if (pLoc.startsWith(".") && pLoc.endsWith(".") && pDom.startsWith(".") && pDom.endsWith("."))
			{
				System.out.println("No es válido, empieza o termina con un \".\" ");
			}
			 else {
				for(int x=0; x<pLoc.length(); x++) {
					lLoc= pLoc.charAt(x);
					
					//Si es diferente de Ñ, haga lo siguiente
					if(lLoc !='Ñ' && lLoc!='ñ' ) {
						
					//Solo puede contener esos caracteres
					if(lLoc>'A' && lLoc<'Z' && lLoc>'0' && lLoc<'9' &&lLoc>'a' && lLoc<'z' && lLoc=='-' && lLoc=='_' && lLoc=='.' ) {
						System.out.println("El caracter "+ lLoc + " no es válido");
					}else {
						System.out.println("El caracter "+ lLoc + " es valido");
					}
					}else {
					System.out.println("Contiene Ñ en la parte local, no es valido");
					}
			}
				for(int y=0;y<pDom.length();y++) {
					...
					if (lDom!='Ñ' && lDom!='ñ') {
						if(lDom>'A' && lDom<'Z' && lDom>'0' && lDom<'9' &&lDom>'a' && lDom<'z' && lDom=='-' && lDom=='.' ) {
							System.out.println("El caracter "+ lDom + " no es válido");
						}else {
							System.out.println("El caracter "+ lDom + "es válido");
						}		
				}else {
					System.out.println("Contiene Ñ en la parte de dominio, no es valido");
				}
			}
		}
	}
